...
export default function TableToolbar(props: Props) {
  ...
  return (
    <div className="flex flex-wrap gap-2">
      ...
      <Select.Root
        value={company ?? ''}
        onValueChange={(v) => onCompanyChange(v || undefined)}
      >
        <Select.Trigger
        ...
        >
          ...
          >
            <Select.ItemText className="px-3 py-2 dark:text-neutral-100 rounded-md cursor-pointer hover:bg-indigo-100  transition">
              All companies
            </Select.ItemText>
          </Select.Item>
      ...
      <Select.Root
        value={sort ?? ''}
        onValueChange={(v: SortValue) => onSortChange(v || undefined)}
      >
        <Select.Trigger
          className="border border-gray-400 rounded-md px-4 py-2 w-48 text-left font-medium 
             bg-gradient-to-r from-indigo-50 to-indigo-100 dark:from-neutral-800 dark:to-neutral-700 
